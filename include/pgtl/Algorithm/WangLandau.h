...
namespace PGTL {
	...
	<names::Algorithm::WangLandau<System_>, names::Algorithm::default_strategy> {
		...
		bool is_known_forbidden_state(size_t i) const noexcept {
			if (i == 1) return true;
			for (size_t n = 0; n < DIMENSIONALITY - 1; n++)
				if (i == (state.system.size - (2*n + 1))) return true;
		...
		bool visits_are_flat() const noexcept {
			...
			size_t threshold = std::floor(params.flatness_threshold*moved/visited.size());

			for (Index i = 0; i < state.energy_levels; i++) {
				if (is_known_forbidden_state(i)) continue;
