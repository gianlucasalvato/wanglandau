...
The program accepts the following options:\footnote{One can also see the manpage by running \verb|man ./wl.1| from the source root directory.}
...
The notebook provides simple functions to load the generated data and compare it with the exact result.
...
\begin{itemize}
	\item[-] The \verb|System| template class accepts one dummy class template argument and is meant to represent the basic, state-independent properties of the system under consideration, such as the dimensionality and the underlying structure of the lattice representation. The template argument represents a name for the system under consideration.\footnote{Such objects are commonly referred to as ``tags'' as they provide no data and no instance functions and are actually compiled out of the program at an early stage; they are only used to name a specific template specialization.}
	...
	This includes a \verb|dof| (degrees of freedom) variable along with an \verb|energy()| function and some utility functions which act on on (or extract information from) the \verb|dof|s.
...
The details of the algorithm can be found in the paper by Landau, Tsai and Exler \cite{bib:wang_landau} as only those implementation-dependent parts are explained below in order for the reader to understand how to use the code. All the relevant parts of the algorithm are coded in the \verb|move()| instance function of the \verb|Algorithm| class.
...
After carefully testing each component of the program a very specific set of tests has been chosen. The only differences between any two tests are the size of the system $N=L^2$ (where $L$ is the length of a single edge of the square simulation box) and the flatness threshold $t$ for the histogram of visited energies.
...
\begin{multicols}{2}
	...
	\pict{cv64}{Base 10 logarithm of $\varepsilon[c_v(T)]$ for $L=64$.}
\end{multicols}
